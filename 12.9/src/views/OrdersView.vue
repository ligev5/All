<template>
  <div class="box">
    ...
    <el-table :data="list" stripe style="width: 100%">
      ...
      <el-table-column label="是否付款">
        <template slot-scope="scope">
          <el-button type="primary" v-show="scope.row.order_pay == '1'">已付款</el-button>
          <el-button type="danger" v-show="scope.row.order_pay == '0'">未付款</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="is_send" label="是否发货"> </el-table-column>
      <el-table-column label="下单时间">
        <template slot-scope="scope">
          {{ scope.row.update_time | filterTime }}
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="dialogFormVisible1 = true"></el-button>
          <el-button
            type="success"
    ...
    <el-dialog title="物流进度" :visible.sync="dialogFormVisible">
      <el-timeline :reverse="reverse">
        <el-timeline-item v-for="(item, index) in arr" :key="index" :timestamp="item.time">
          {{ item.context }}
          {{ item.time }}
        </el-timeline-item>
      </el-timeline>
    ...
    <el-dialog title="修改地址" :visible.sync="dialogFormVisible1">
      <el-form :model="form1">
        <el-form-item label="省市区/县" :label-width="formLabelWidth">
          <el-cascader :options="arrCity" :props="{}" clearable></el-cascader>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    search() {
      this.tableData = this.tableList.filter((i) => i.order_number.includes(this.value))
    }
  },
  created() {
    this.$http.get('./api/orders', { pagesize: this.pagesize, pagenum: this.pagenum }).then((res) => {
      console.log(res)
